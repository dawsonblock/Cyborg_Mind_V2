...
3. (Optional, for full Minecraft training) Use a Python 3.10 environment and install the `minecraft` extras: `pip install '.[rl,minecraft,clip,tutor]'`.  The `minerl` and `minedojo` packages are heavy and are not included in `requirements.txt` by default.
...
```bash
./run_api.sh          # uses local .venv if present
...
```bash
./run_gui.sh          # uses local .venv if present
